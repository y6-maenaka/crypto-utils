...
/**
 * @brief AES-CBC mode encryption/decryption
 *
...
public:
     ...
     *
     * Contains ciphertext and IV (both needed for decryption)
     */
    ...
    static core::result<encrypted_data> encrypt(
        ...
        // Generate random IV
        auto iv_result = iv_generator::generate_aes_iv();
        if (!iv_result) {
            return iv_result.error_value();
        }
        core::byte_vector iv = std::move(*iv_result);
        ...
        core::byte_vector ciphertext(plaintext.size() + aes_block_size);
        int outlen = 0;
        ...
        return encrypted_data{std::move(ciphertext), std::move(iv)};
    }

    /**
     * @brief Decrypt data with AES-CBC
     *
     * @param enc_data Encrypted data (ciphertext + IV)
     * @param key AES key
    ...
    static core::result<core::byte_vector> decrypt(
        ...
        if (enc_data.iv.size() != aes_iv_size) {
            return core::error(
        ...
        if (EVP_DecryptFinal_ex(
            ...
            return core::error(
                core::error_code::decryption_failed,
                "Failed to finalize decryption (wrong key or corrupted data?)"
            );
        }
...
using aes_128_cbc = aes_cbc<aes_key_size::aes_128>;
using aes_192_cbc = aes_cbc<aes_key_size::aes_192>;
using aes_256_cbc = aes_cbc<aes_key_size::aes_256>;
